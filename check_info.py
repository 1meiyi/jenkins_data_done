...
class Autoinstall:
    ...
    def get_allure_info(self, path, info):
                    ...
                    if 'test_musa_cuda_samples' in test_case['name']:
                        if test_case['status'] == 'failed':
                            ...
                            if count_sample_fail > 0 and count_sample_fail != 0:
                                self.log.error(f'failed {info}-{rs_path1} failed : {count_sample_fail} 条')
                                failed_case = test_case['name']
                                self.log.info(f'error {info} {rs_path1} 失败测试用例：{failed_case}')
                            elif count_sample_pass > 0 and count_sample_pass != 0:
                                self.log.info(f'scuess {info}-{rs_path1} passed : {count_sample_pass} 条')
                                self.log.info('--------------------------------------------')
                    else:
        ...
        if count_fail > 0:
            ...
            failed_case = test_case['name']
            self.log.info(f'{info} 失败测试用例：{failed_case}')
        elif count_broken > 0:
